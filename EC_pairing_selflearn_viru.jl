...
@time begin
  Threads.@threads for index_2D in CartesianIndices((3,3))
    hh_hh[Tuple(index_2D)[1],Tuple(index_2D)[2]]= apply(swapprime(dag(hh[Tuple(index_2D)[1]]),0 => 1), hh[Tuple(index_2D)[2]]; cutoff=1e-30)
    #@show maxlinkdim(hh_hh[Tuple(index_2D)[1],Tuple(index_2D)[2]])
  end
end
using_threaded_blocksparse = ITensors.enable_threaded_blocksparse()
...
for dim_EC in 1:dim_EC_max
  ...
  ψ_ec[dim_EC]=richardson_dmrg(sites,G_EC)
  ...
  @time begin
      Threads.@threads for index_3D in CartesianIndices((dim_EC,3,3))
        ...
        h_mat[Tuple(index_3D)[2],Tuple(index_3D)[3],dim_EC,Tuple(index_3D)[1]]=
                          inner(prime(ψ_ec[dim_EC]),hh_hh[Tuple(index_3D)[2],Tuple(index_3D)[3]],ψ_ec[Tuple(index_3D)[1]])
